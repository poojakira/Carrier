...
import { GreenhouseAdapter } from '@/lib/adapters';
...
const schema = z.object({ boardToken: z.string().trim().min(1), company: z.string().trim().min(1) });
...
export async function POST(req: Request) {
  try {
    ...
    const jobs = await new GreenhouseAdapter(d.boardToken).listJobs();
    let count = 0;
    const highMatchJobs: { title: string; company: string; matchScore: number }[] = [];

    for (const j of jobs) {
      const scores = calculateMatch(
        { ...j, source: 'greenhouse', salaryMin: j.salaryMin ?? null, salaryMax: j.salaryMax ?? null, remote: j.remote ?? false, sponsorship: j.sponsorship ?? null, trustScore: 75, requirementsJson: JSON.stringify(j.requirements || []), riskFlagsJson: '[]', technicalFit: 0, experienceFit: 0, educationFit: 0, projectFit: 0, locationFit: 0, compensationFit: 0, authorizationFit: 0, seniorityFit: 0, careerValueFit: 0, overallFit: 0, id: '', userId: null, createdAt: new Date(), updatedAt: new Date() },
      ...
      await db.job.upsert({
        where: { source_externalId: { source: 'greenhouse', externalId: j.externalId } },
        update: { company: d.company, title: j.title, location: j.location, url: j.url, description: j.description, salaryMin: j.salaryMin, salaryMax: j.salaryMax, remote: j.remote ?? false, sponsorship: j.sponsorship ?? null, ...scores, requirementsJson: JSON.stringify(j.requirements || []) },
        create: { userId: u.id, source: 'greenhouse', externalId: j.externalId, company: d.company, title: j.title, location: j.location, url: j.url, description: j.description, salaryMin: j.salaryMin, salaryMax: j.salaryMax, remote: j.remote ?? false, sponsorship: j.sponsorship ?? null, trustScore: 75, ...scores, requirementsJson: JSON.stringify(j.requirements || []), riskFlagsJson: '[]' },
      });
    ...
    await audit(u.id, 'JOB_INGESTED', 'source', d.boardToken, { provider: 'greenhouse', count });
